...
void (*TestFunctions[])(iSeries::DoneCB_t cb) = {
    ics0, ics1, ice0, ice1, dcs0, dcs1, dce0, dce1
};
const size_t nTestFunctions = sizeof(TestFunctions)/sizeof(void (*)(iSeries::DoneCB_t cb));
...
void ei(iSeries::DoneCB_t cb, int i)
{
    (void) i;
    if(SeriesFuncsTrace) printf("%s, seq %d\r\n", __PRETTY_FUNCTION__, seq);
...
void ed(iSeries::DoneCB_t cb, int i)
{
    (void) i;
    if(SeriesFuncsTrace) printf("%s, seq %d\r\n", __PRETTY_FUNCTION__, seq);
    int rc = (errMax <= Actual.errors)?1:0;
